...
namespace {
    ...
    const string kStateShapeDir = "res/Elections/Visualizer";
    const string kStateFilename  = "us-borders.txt";
    ...
    void loadStateShapes() {
        ...
        absPath = absPath + "/" + kStateShapeDir + "/" + kStateFilename;
        ...
        string fullname1 = kStateShapeDir + "/" + kStateFilename;
        ...
        string fullname2 = kStateShapeDir.substr(kStateShapeDir.find("/") + 1) + "/" + kStateFilename;

        /* First, try to open the file using the reduced relative path name (for windows). */
        {
        ...
        {
            ifstream fileReader(kStateFilename);
            if (fileReader) {
                input = std::move(fileReader);
            }
        }
    ...
    void showStats(int year, int totalPopularVote, int wonPopularVote) {
        /* We have to use a GText to render text. Just make one and reuse it. */
        unique_ptr<GText> label(new GText());
