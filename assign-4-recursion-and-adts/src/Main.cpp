...
namespace {
    ...
    string haveUserChooseFile(ifstream& input, const string& prompt, const string& directory) {
        while (true) {
            ...
            string fullname = "res/" + directory + "/" + filename;

            /* First, try to open the file using the updated relative path name. */
            {
    ...
    HospitalTestCase userChooseHospitalTest() {
        while (true) {
            ifstream input;
            string filename = haveUserChooseFile(input, "输入文件名: ", "DoctorsWithoutOrders");
    ...
    DisasterTest userChooseDisasterTest() {
        while (true) {
            ifstream input;
            string filename = haveUserChooseFile(input, "请输入文件名: ", "DisasterPlanning");
    ...
    void processDisasterTest(const DisasterTest& test) {
        ...
        displayNetwork(test);
        cout << "库存以黄色显示，覆盖的城市以白色显示。" << endl;

        while (true) {
            string input = getLine("输入多个城市，或按Enter键结束: ");
    ...
    void confirmDisasterOptimality(const DisasterTest& test) {
        ...
        setCitiesToStockpile(bestCities, true);
    ...
    bool processMenuSelection(MenuOption option) {
        switch (option) {
    ...
    void printMenu() {
        cout << "您可选的方式有: " << endl;
        cout << "  " << TestDoctorsWithoutOrders         << ": 没有医嘱的测试医生" << endl;
        cout << "  " << TestDisasterPreparation          << ": 测试灾难准备" << endl;
        cout << "  " << TestOptimalDisasterPreparation   << ": 查找灾难的最小覆盖范围" << endl;
        cout << "  " << TestSimpleElections              << ": 在简化测试用例上测试赢得选举" << endl;
        cout << "  " << TestWinningTheElection           << ": 根据历史数据推测选举获胜" << endl;
        cout << "  " << ShowOffWinningTheElection        << ": 查看历史选举模式" << endl;
        cout << "  " << Quit                             << ": 退出" << endl;
    }
    ...
    MenuOption getMenuSelection() {
        while (true) {
            MenuOption option = MenuOption(getInteger("输入您的选择: "));
...
int main() {
    do {
        printMenu();
    } while (processMenuSelection(getMenuSelection()));
