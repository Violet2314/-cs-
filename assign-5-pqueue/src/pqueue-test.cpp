...
static PQueue *buildPQueue(PQueue::PQueueType pqtype, Iterable iter, int size, Vector<string>& insertedElems) {
    PQueue *pq = PQueue::createPQueue(pqtype);
    int count = 0;
    for (const string& key: iter) {
        pq->enqueue(key);
        insertedElems.add(key);
        count++;
        if (count == size) break;
    }
    ...
    if (pq->size() == 0) {
        error("Looks like " + PQueue::typeToName(pqtype) + "::enqueue " +
              "isn't implemented properly, because your queue says " +
              "it's empty, and that shouldn't be the case.");
    }

    cout << "+ Inserted " << pq->size() << " words." << endl;

    // Now keep insertedElems in sync with expected PQ order.
    // Would use a Set instead of a Vector for insertedElems,
    // but it's possible to have inserted duplicates into the PQ
    std::sort(insertedElems.begin(), insertedElems.end());
 ...
 */
static void extractElements(PQueue::PQueueType pqtype, PQueue *pq, const Vector<string>& expectedElems) {
    ...
    while (!pq->isEmpty()) {
        ...
        if(next != expectedElems[count]) error("Priority Queue is returning elements in the wrong order." + to_string(count) + next + " " + expectedElems[count]);
        count++;
        if (count % 1000 == 0) {
            cout << ".";
            cout.flush();
...
static void sortedTest(PQueue::PQueueType pqtype, const Lexicon& english, int size) {
    ...
    PQueue *pq = buildPQueue(pqtype, english, size, inserted);
...
static void randomizedTest(PQueue::PQueueType pqtype, int size) {
    ...
    Vector<string> inserted;
    PQueue *pq = buildPQueue(pqtype, randomStrings, size, inserted);
    extractElements(pqtype, pq, inserted);
    delete pq;
...
static void mergeTest(PQueue::PQueueType pqtype, const Lexicon& english, int size) {
    ...
    PQueue *pq1 = buildPQueue(pqtype, set, size, unused);
    PQueue *pq2 = buildPQueue(pqtype, set, size, unused);
    int size1 = pq1->size();
    int size2 = pq2->size();
    if (size1 == 0 || size2 == 0) {
